...
describe('Matrix component', () => {
    ...
    it('should render', () => {
        render(<Matrix socket={socket}/>);
    });
    it('should have "Minesweeper" as title', () => {
        const { getByText } = render(<Matrix socket={socket}/>);
        expect(getByText('Minesweeper')).toBeInTheDocument();
    });
    it('should render 10x10 matrix of buttons', () => {
        const { getAllByRole } = render(<Matrix initialRows={10} initialCols={10} socket={socket}/>);
        expect(getAllByRole('button')).toHaveLength(100);
    });
    it('should render 10 mines', () => {
        const { getAllByText } = render(<Matrix initialRows={10} initialCols={10} initialMines={10} socket={socket}/>);
    ...
    it('should remove child hidden class when some button is clicked', async () => {
        const { getAllByRole } = render(<Matrix initialRows={10} initialCols={10} initialMines={10} socket={socket}/>);
    ...
    it('should show "You lose" when a mine is clicked', async () => {
        const { getAllByText, getByText } = render(<Matrix initialRows={10} initialCols={10} initialMines={10} socket={socket}/>);
        const mines = getAllByText('💣');
    ...
    it('should show nearest mines counter inside cell when random cell is clicked', async ()=>{
        const { getAllByRole, getAllByText } = render(<Matrix initialRows={10} initialCols={10} initialMines={10} socket={socket}/>);
        const cells = getAllByRole('cell');
